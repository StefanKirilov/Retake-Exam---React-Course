...
export default function Sushi() {
    ...
    const { id } = useParams();
    const { shopping, getItemQuantity } = useShoppingCart();
        ...
        <div className={styles.top}>
            <div className={styles['container-first']}>
                <div className={styles['container-img']}>
                    <img className={styles.image} src={sushi.image} />
                </div>
                <div className={styles['container-items']}>
                    <p className={styles.price}>{sushi.price} лв.</p>
                    <p className={styles.weight}>/ {sushi.weight} гр.</p>
                    <p className={styles.count}>/ {sushi.count} бр.</p>
                    <h1 className={styles.name}> {sushi.name} </h1>
                    <div className={styles.btn}>
                        <button onClick={minus}>-</button>
                        <p className={styles.number}>{count}</p>
                        <button onClick={plus}>+</button>
                        <Link onClick={() => shop(sushi._id, count)} className={styles.shopCar} to=""><img src={shopCar} alt="" /></Link>
