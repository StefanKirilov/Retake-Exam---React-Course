...
export default function Footer() {
    ...
    const deleteComment = async (id) => {
        handleDialog('Сигурни ли сте, че искате да изтриете коментара?', true);
        idCommentRef.current = id;
    }

    const areUSureDelete = async (choose) => {
        if (choose) {         
            await apiService.deleteComment(idCommentRef.current);
            setChange(!change);
